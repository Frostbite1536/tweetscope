...
def compute_target_leaf_topics(
    n_rows: int,
    median_text_chars: float = 180.0,
    reply_ratio: float = 0.5,
) -> int:
    """Corpus-density-aware heuristic for target leaf-topic count.

    Based on the design doc Section 2: text density and standalone ratio
    determine how many rows each leaf cluster should contain, which converts
    to a target leaf count clamped by corpus-size band.

    Args:
        n_rows: Total number of rows in the corpus.
        median_text_chars: Median character length of texts.
        reply_ratio: Fraction of rows that are replies (0.0 to 1.0).

    Returns:
        Target number of leaf-level clusters for PLSCAN ``base_n_clusters``.
    """
    text_density = max(0.0, min(1.0, (median_text_chars - 120) / 120))
    standalone_density = max(0.0, min(1.0, ((1 - reply_ratio) - 0.15) / 0.30))
    ...
    if n_rows <= 5_000:
        rows_per_leaf = _lerp(55, 25, corpus_density)
        lo, hi = 16, 200
    elif n_rows <= 50_000:
        rows_per_leaf = _lerp(130, 55, corpus_density)
        lo, hi = 80, 500
    else:
        rows_per_leaf = _lerp(220, 100, corpus_density)
        lo, hi = 250, 1600

    target = round(n_rows / rows_per_leaf)
    return max(lo, min(hi, target))


def compute_target_leaf_topics_from_input_df(
    input_df: pd.DataFrame,
    *,
    text_column: str,
    n_rows: int | None = None,
) -> tuple[int, float, float]:
    """Compute auto ``base_n_clusters`` from the configured text column.

    Returns ``(target_leaf_topics, median_text_chars, reply_ratio)``.
    """
    if text_column not in input_df.columns:
        raise KeyError(
            f"Configured text column '{text_column}' was not found in input.parquet"
        )

    effective_n_rows = int(n_rows) if n_rows is not None else int(len(input_df))
    median_chars = float(input_df[text_column].astype(str).str.len().median())
    reply_ratio = 0.0
    if "is_reply" in input_df.columns:
        col = input_df["is_reply"]
        if col.dtype == object:
            reply_ratio = float(col.astype(str).str.lower().isin(["true", "1"]).mean())
        else:
            reply_ratio = float(col.astype(bool).mean())
    elif "tweet_type" in input_df.columns:
        reply_ratio = float((input_df["tweet_type"] == "reply").mean())

    target = compute_target_leaf_topics(
        n_rows=effective_n_rows,
        median_text_chars=median_chars,
        reply_ratio=reply_ratio,
    )
    return target, median_chars, reply_ratio
